...
const Dashboard = () => {
  ...
  const handleAddCategory = async () => {
    ...
    // Set the categoryAdded state to true
    setCategoryAdded(true);

    } catch (error) {
      console.error("Error adding category:", error);
      // Set the categoryError state to true
  ...
  const handleDeleteCategory = async (categoryId) => {
    try {
      ...
      if (response.data.success) {
        ...
        // Set the categoryDeleted state to true
  ...
  const handleSaveChanges = async (categoryId) => {
    ...
    const addItem = () => {
      ...
      .then(data => {
        ...
        // Set the productAdded state to true
        setProductAdded(true);
      })
      .catch(error => {
        console.error('Error adding item:', error);
  
        // Set the productError state to true
        ...
        <div className="row">
          ...
          <div className="col-xs-12">
            <div className="add-cat-container">
              ...
              {categoryAdded && <p className="added-alert" >تمت إضافة قائمة جديدة</p>}
              {/* Display error message if there was an error adding the category */}
              {categoryError && <p className="error-alert" >حدث خطأ في الإضافة. برجاء تحميل الصفحة.</p>}
            </div>
          </div>
          ...
          <div className='col-xs-12'>
            ...
            <div>
                ...
                <select
                  ...
                  {productAdded && <p className="added-alert">تمت إضافة منتج جديد </p>}
                  
                  {/* Display error message if there was an error adding the product */}
                  {productError && <p className="error-alert">حدث خطأ في إضافة المنتج. برجاء تحميل الصفحة.</p>}
            </div>
                    ...
                    {editingStates[cat.id] ? (
                      ...
                      <>
                        ...
                        {categoryDeleted[cat.id] && (
                          <span className="delete-success-message">تم الحذف</span>
                        )}
                      ...
                      <td>
                        <div className="delete-and-edite-btns">
                          <div className="delete-btn-content">
                            ...
                            {deleteMessages[item.id] && <h4>{deleteMessages[item.id]}</h4>}
                          </div>
                          <div className="edit-btn-content">
                            <button className="table-list-btn btn-icon bg-primary"><AiFillEdit /></button>
                          </div>
                        </div>
